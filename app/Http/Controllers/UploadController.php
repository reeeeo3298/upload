...
//        dd($results);
     ...
     * 都市ガスお客様一覧画面表示
     */
    public function list_citygas_index(){
                
        //データ取得
        $results = $this->data_citygas_all();
       
        return view('main_citygas.index',compact('results'));
    }
...
//        $results = $this->data_all();
     ...
     * 都市ガスお客様検索処理
     */
    public function search_citygas_index(Request $request){
        
        $all = $request->all();
        
        //セッション追加
        session(['search_data_citygas' => $all]);
        ...
        $list = DB::table('t_data_citygas as td');
//                    ->join('m_area as ma','td.area_cd','=','ma.area_id');
...
//            dd($list);
     ...
     * 都市ガスお客様情報csv出力
     */
    public function csv_citygas_index(Request $request){
        
        $search_data = $request->session()->get('search_data_citygas');
...
//            dd($list);
        ...
        $headers = array(
            'Content-Type' => 'text/csv',
            'Content-Disposition' => 'attachment; filename="customerlist_citygas'.$now_dt.'.csv"',
        );
...
//        dd($file_type);
     ...
     * 都市ガス登録画面表示
     */
    public function add_citygas_index(Request $request){
        
        $accept_no = $request->input('accept_no');
        
        $results = DB::table('t_data_citygas as td')
//                    ->join('M_contacttype as mt','tc.type_id','=','mt.type_id')
//                    ->join('M_status as ms','tc.status_id','=','ms.status_id')
//                    ->join('M_staff as mst','tc.staff_cd','=','mst.staff_cd')
                    ->where('accept_no',$accept_no)
...
//        dd($file_type);
        ...
        return view('add_citygas.index',compact('accept_no','results','file_type'));
    }
...
//        dd($all);
     ...
     * 都市ガス登録処理
     */
    public function add_citygas_input(Request $request){
...
//        dd($all);
     ...
     * 都市ガス登録保留処理
     */
    public function hold_citygas_input(Request $request){
...
//                    ->where('ms.status_id','1');
     ...
     * 都市ガスデータ取得
     */
    private function data_citygas_all(){
...
//                    ->join('M_staff as mst','tc.staff_cd','=','mst.staff_cd')
     ...
     * 都市ガス詳細データ取得
     */
    public function data_citygas_detail(Request $request){
